...
export function Step({ children }: StepProps): JSX.Element {
  return <div className="step-default" style={{
    height: '400px'
  }}>{children}</div>;
}
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#a55610e1', color: '#f3ceb3' },
          active: { scale: 1, backgroundColor: '#f4ddb9', color: '#8e510f' },
          complete: { scale: 1, backgroundColor: '#5736055a', color: '#f7dfca' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: '100%', backgroundColor: '#81530e' }
  };
...
function CheckIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg {...props} fill="none" stroke="currentColor" strokeWidth={5} viewBox="0 0 25 25">
      <motion.path
